...
export class CameraEmailProvider implements EmailProvider {
 async invite(email:string,url:string):Promise<'smtp'|'demo-inbox'>{
  ...
  if(process.env.SMTP_URL){
   await nodemailer.createTransport(process.env.SMTP_URL).sendMail({from:process.env.SMTP_FROM||'demo@localhost',to:email,subject:'DigitalAqtau: добровольное подключение камеры',text:body});
  }
