...
import './globals.css';
// Шрифты скачиваются при сборке и отдаются с нашего сервера. Внешний @import в CSS блокировал
...
export default function RootLayout({ children }: { children: React.ReactNode }) { return <html lang="ru" className={`${onest.variable} ${unbounded.variable}`}><body>{children}<LocaleBridge/><script dangerouslySetInnerHTML={{ __html: "if('serviceWorker' in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js').catch(()=>{}))}" }} /></body></html>; }
