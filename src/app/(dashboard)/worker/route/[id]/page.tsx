...
export default async function RoutePage({params}:{params:Promise<{id:string}>}){
 const u=await requireUser('WORKER');const {id}=await params;if(!z.string().uuid().safeParse(id).success)notFound();const task=await db.workerTask.findUnique({where:{incidentId:id},include:{incident:true}});if(!task||task.workerId!==u.id)notFound();
