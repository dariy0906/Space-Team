...
import { sourceLabel, taskLabel, typeLabel } from '@/lib/labels';
...
export default async function IncidentDetail({ params,searchParams }: { params: Promise<{id:string}>; searchParams:Promise<{toast?:string}> }) {
  ...
  return <>
    ...
    <div className="content-grid"><div className="space-y-4">
      <section className="panel"><div className="detail-hero"><span className="incident-icon text-xl"><TypeGlyph type={incident.type} size={20} animate/></span><div className="flex-1"><div className="flex flex-wrap gap-2 mb-2"><SeverityBadge severity={incident.severity}/>{incident.isDemo&&<span className="badge status-new">DEMO / MOCK</span>}</div><p className="text-sm">{incident.description||'Описание не указано'}</p></div></div><div className="panel-body border-t" style={{borderColor:'var(--line)'}}><div className="detail-facts"><div><span>Категория</span>{typeLabel[incident.type]}</div><div><span>Источник</span>{sourceLabel[incident.source]}</div><div><span>Достоверность</span>{incident.confidence===null?'—':`${incident.confidence}%`}</div><div><span>Исполнитель</span>{incident.assignedWorker?.name||'Не назначен'}</div><div><span>Координаты</span>{incident.lat.toFixed(5)}, {incident.lng.toFixed(5)}</div><div><span>Микрорайон</span>{incident.district||'—'}</div></div>{incident.media.map(m=><div key={m.id}>{m.type==='IMAGE'?<Image src={m.url} alt="Фото события" width={800} height={500} unoptimized className="media-preview"/>:<a href={m.url} className="text-link">Видео события</a>}</div>)}</div></section>
