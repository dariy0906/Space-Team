...
const b=await chromium.launch({...(process.env.CHROMIUM_PATH?{executablePath:process.env.CHROMIUM_PATH}:{}),headless:true,args:['--no-sandbox','--use-fake-device-for-media-stream','--use-fake-ui-for-media-stream']});
const hash=t=>createHash('sha256').update(t).digest('hex');
async function until(fn,timeout=10000){const start=Date.now();for(;;){let v;try{v=await fn();}catch{}if(v)return v;if(Date.now()-start>timeout)throw new Error('poll timeout');await new Promise(r=>setTimeout(r,200));}}
...
async function login(email,role){
 ...
 await p.waitForURL('**/'+role,{waitUntil:'domcontentloaded'});return {ctx,p};
}
async function invite(admin){
 for(let attempt=0;attempt<3;attempt++){
  await admin.goto(origin+'/admin/cameras',{waitUntil:'domcontentloaded'});
  const form=admin.locator('form').filter({has:admin.locator('input[name=cameraId]')}).first();
...
try{
 ...
 assert.equal(await operator.getByRole('button',{name:/Включить|Начать|Start/i}).count(),0,'оператор не управляет включением камеры');

 // --- Сценарий 1: owner STOP ---
 const token1=await invite(admin);
 const {p:phone1}=await phoneFor(token1);
 assert.equal(await phone1.locator('video').evaluate(v=>v.srcObject),null,'до согласия видео не передаётся');
 let s=await db.cameraSession.findUniqueOrThrow({where:{tokenHash:hash(token1)}});
 assert.equal(s.status,'INVITED');
 await phone1.getByRole('button',{name:'Разрешить использовать мой телефон как demo camera',exact:true}).click();
 await phone1.getByText(/камера передаёт видео/).waitFor();
 ...
 await operator.waitForFunction(()=>document.querySelector('video')?.readyState>=2);
 ...
 await until(async()=> (await db.cameraSession.findUniqueOrThrow({where:{tokenHash:hash(token1)}})).status==='STOPPED');
 ...
 await phone2.getByRole('button',{name:'Разрешить использовать мой телефон как demo camera',exact:true}).click();
 ...
 assert.equal(await phone2.locator('video').evaluate(v=>v.srcObject.getTracks()[0].readyState),'ended');
 await until(async()=> (await db.cameraSession.findUniqueOrThrow({where:{id:s2.id}})).status==='STOPPED');
 console.log('PASS admin revocation stops phone tracks');

 // --- Сценарий 3: email-приглашение жителю (demo-inbox) ---
 await admin.goto(origin+'/admin/cameras',{waitUntil:'domcontentloaded'});
 const inviteForm=admin.locator('form').filter({has:admin.locator('input[name=cameraId]')}).first();
 ...
 const note=await until(async()=>db.notification.findFirst({where:{userId:resident2.id,href:{startsWith:'/camera/'},createdAt:{gt:new Date(Date.now()-60000)}}}),20000);
...
}catch(e){console.error('CAMERA E2E FAILED',e);throw e;}finally{await b.close();await db.$disconnect();}
